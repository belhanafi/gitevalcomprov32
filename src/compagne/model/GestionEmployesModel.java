	...
	public List loadListEmployes() throws SQLException{
		...
		try {
			...
			String sql_query="select login,concat(c.nom,'-',c.prenom) as nom, id_employe,date_naissance,date_recrutement ,concat(d.niv_for_libelle,'-',libelle_diplome) as libelle_formation,p.intitule_poste, email,"
					...
					+ " case    when length(trim(libelle_direction))=0 then 'DIR NON RENSEIGNEE'  else libelle_direction end as libelle_direction,t.structure_ent libelle_structure,sexe_lbl,type_contrat_lbl,evolution_carriere,antecedent_disciplinaire,csp"
	...
	public boolean addEmploye(GestionEmployesBean addedData) throws ParseException
	{
		...
		try 
		{
			...
			String sql_query="INSERT INTO employe( nom, prenom, date_naissance, rattach_dg, date_recrutement, code_formation, code_poste, email, est_evaluateur, est_responsable_service, est_responsable_direction, est_responsable_division, est_responsable_departement, est_responsable_unite, est_responsable_section, est_responsable_rh, code_structure, id_compte,code_sexe,code_contrat,evolution_carriere,antecedent_disciplinaire,csp)" +
					" VALUES(#nom,#prenom,#date_naissance,'N',#date_recrutement,#code_formation,#code_poste,#email,#est_evaluateur,'N','N','N','N','N','N',#est_responsable_rh,#code_structure,#id_compte,#sexe,#type_contrat,#evolution_carriere,#antecedent_disciplinaire,#csp)";
			...
			sql_query = sql_query.replaceAll("#csp","'"+addedData.getCsp()+"'");
	...
	public Boolean updateListeEmploye(GestionEmployesBean addedData)
	{
		...
		try 
		{
			...
			String sql_query="update employe  set nom=#nom, prenom=#prenom, date_naissance=#date_naissance, rattach_dg='N', date_recrutement=#date_recrutement, code_formation=#code_formation, code_poste=#code_poste, email=#email, est_evaluateur=#est_evaluateur, est_responsable_service='N', est_responsable_direction='N', est_responsable_division='N', est_responsable_departement='N'," +
					...
					" code_structure=#code_structure, id_compte=#id_compte, code_sexe=#code_sexe,code_contrat=#code_contrat,evolution_carriere=#evolution_carriere,antecedent_disciplinaire=#antecedent_disciplinaire,csp=#csp where id_employe=#id_employe " ;
	...
	public List  filtreEmployes(String condition1, String valeur_condition1, String condition2,  String valeur_condition2) throws SQLException, ParseException{
		...
		try {
			...
			sel_comp="select login,concat(c.nom,'-',c.prenom) as nom, id_employe,date_naissance,date_recrutement ,concat(d.niv_for_libelle,'-',libelle_diplome) as libelle_formation,p.intitule_poste, email,  CASE WHEN est_evaluateur='N' THEN 'NON' ELSE 'OUI' END as est_evaluateur,"
					...
					+ "	 else libelle_direction end as libelle_direction,evolution_carriere,antecedent_disciplinaire,csp"
