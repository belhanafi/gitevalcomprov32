	 ...
	 *//*
	  ...
	  *//************//*
		...
		catch (SQLException e) 
		{
			// TODO Auto-generated catch block
			e.printStackTrace();
			System.out.println("Exception lors de l'intégration des compte (grande boucle)");
		}
...
//			updateMultiQueryCommon(requete);
		...
		//construction de la requete et insertion dans la base de données
	...
	/******
	 * 
	 * Modifier l'annéée de naissance par un entier aleatoire pour eviter les doublons de logins d'utilisateurs existants en basecréées avec lés précedents
	 * *****/
	...
	public String addCompte(String requete,EmployeCompteBean donneeBean)
	{
		...
		String log=donneeBean.getLogin().replaceAll(" ", "");
		ApplicationSession applicationSession =(ApplicationSession) Sessions.getCurrent().getAttribute("APPLICATION_ATTRIBUTE");
		Integer database_id=applicationSession.getClient_database_id(); //recuperation de la base de donnée actuelle
		//TODO supprimer cette ligne ApplicationFacade
	...
	public void updateMultiQueryCommon(String requete)
	{
		...
		if(requete!="")
		{
			...
			try 
			{
				...
				System.out.println("requete qui génère l'exception="+requete);
				e.printStackTrace();
