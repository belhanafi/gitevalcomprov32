...
public class StructureEntreprise extends GenericForwardComposer {
	...
	Textbox codeUnite;
	Textbox nomUnite;
	Textbox codesousDirection;
	Textbox nomsousDirection;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		codeDivision.setDisabled(false);
		codeDirection.setDisabled(false);
		codeUnite.setDisabled(false);
		codesousDirection.setDisabled(false);
		codeDepartement.setDisabled(false);
		codeService.setDisabled(false);
		codeSection.setDisabled(false);
	...
	public void onClick$add() throws WrongValueException, ParseException, SQLException {
		...
		codesousDirection.setValue(admini_model.getNextCode("K",chaine[5],4));
		codeDepartement.setValue(admini_model.getNextCode("T",chaine[6],4));
		codeService.setValue(admini_model.getNextCode("R",chaine[7],4));
		codeSection.setValue(admini_model.getNextCode("E",chaine[8],4));;
	...
	public void onClick$okAdd() {
		...
		addedData.setCodeSousDirection(getSelectedCodeSDurection());
		addedData.setLibelleSousDirection(getSelectedNomSDurection());
	...
	public void onClick$update() throws InterruptedException {
		...
		selected.setCodeSousDirection(getSelectedCodeSDurection());
		selected.setLibelleSousDirection(getSelectedNomSDurection());
						...
						if(donneeRejetes.size()!=0)
						{
							...
							while(index1.hasNext())
							{
								...
								String donneeString=donnee.getCodestructure()+";"+donnee.getCodeDivision()
										...
										+";"+donnee.getCodeSousDirection()
										+ ";"+donnee.getLibelleSousDirection()
...
//	}
	...
	public void onSelect$structureEntrepriselb() {
		closeErrorBox(new Component[] { codeStructure, codeDivision,nomDivision,codeDirection,  nomDirection, 
				codeUnite,nomUnite, codesousDirection,nomsousDirection,codeDepartement, nomdepatrement, codeService,NomService, codeSection, nomSection });
	}
	...
	private String getSelectedCodeSDurection() throws WrongValueException {
		String name = codesousDirection.getValue();
		//if (Strings.isBlank(name)) {
		if (2==1) {
			throw new WrongValueException(codesousDirection, "le code s/direction ne doit pas être vide!");
		}
		return name;
	}

	private String getSelectedNomSDurection() throws WrongValueException {
		String name = nomsousDirection.getValue();
		//if (Strings.isBlank(name)) {
		if (2==1) {
			throw new WrongValueException(nomsousDirection, "le nom s/direction ne doit pas être vide!");
		}
		return name;
	}
	...
	public void clearFields(){
		...
		codeUnite.setText("");
		nomUnite.setText("");
		codesousDirection.setText("");
		nomsousDirection.setText("");
